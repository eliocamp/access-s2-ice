...
files <- rbind(
  ...
  CJ(time = dates, member = "mm") |>
    _[, file := file.path(
      globals$data_derived, "hindcast_ensmean",
...
read_measures <- function(files) {
  ...
  read <- function(i) {
    ...
    data |>
      ...
      _[] |>
      _[, let(
        time_forecast = dates$time_forecast[[i]],
...
list.files(file.path(globals$data_derived, "rmse"),
  pattern = "di_aice_",
  full.names = TRUE
) |>
  read_measures() |>
...
list.files(file.path(globals$data_derived, "iiee"),
  pattern = "di_aice_",
  full.names = TRUE
) |>
  read_measures() |>
