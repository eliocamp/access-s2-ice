...
extents[model %in% c("access", "hadisst") & variable == "extent"] |> 
...
extents[model %in% c("access", "hadisst") & variable == "extent"] |> 
  ...
  geom_text(data = \(x) x[, cor(extent, sst, use = "complete.obs"), by = model],
            aes(label = scales::number(V1, prefix = "r=")), 
            x = 3, y = -4) +
  facet_wrap(~model, ncol = 1)
```
## Extent vs. SST
...
extents[model %in% c("access", "hadisst") & variable == "extent"] |> 
  ...
  _[model != "access", model := "observations (ERA5 + HadISST)"] |> 
  dcast(time + model ~ variable, value.var = "value") |> 
  _[!is.na(extent), extent_sst := predict(lm(extent ~ sst)), by = model] |> 
